...
public class Person {
	...
	hashCode()에서는 나이를 반환하고, 
	equals() 에서는 이름만 비교하는 형태라면 이상한 결과가 나올수도 있다. 
	 */
	@Override
	public boolean equals(Object obj) {
		Person person=(Person)obj;
		System.out.println("equals메소드 호출됨");
		//if(person.age==this.age&&person.name.equals(this.name)) { //둘다비교
			//if(person.age==this.age){//나이로만 비교 
			if(person.name.equals(this.name)){//이름으로만비교
			return true;
		}
		else {
			return false;
