...
-main()메소드가 실행되기전에 이미 초기화 되므로 main()
메소드 실행시 별도의 선언없이 변수를 사용할 수 있다.
...
접근방법
	...
	-클래스 외부
		...
		둘다 가능함. 단, 클래스명으로 접근하는것을 권장한다.
		
 */
class MyStatic {
	/*
	멤버변수]인스턴스형 멤버변수는 반드시 클래스의 객체를 생성한후 
	참조변수를 통해 접근해야한다.
	 */
	int instanceVar;
	/*
	정적멤버변수] 객체를 생성할 필요없이 클래스명으로 바로 접근할수 있다.
	프로그램 시작시 이미 메소드영역에 로드된다.
	 */
	static int staticVar;
	/*
	인스턴스형 멤버메소드] 인스턴스형 메소드에서는 모든 멤버를 사용할수있다.
	인스턴스형 변수,정적변수 둘다 접근 가능하다.
	 */
	void instanceMethod() {
	...
	/*
	정적 멤버메소드] 정적메소드 안에서는 인스턴스 멤버를 사용할수 없다.
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		/*
		일반적인멤버변수에 접근하는 방법 : MyStatic클래스의
		...
		/*
		static(정적)멤버변수 접근방법
			방법1:클래스명을 통햏서 접근한다. 객체 생성없이
			바로접근이 가능하다.
		 */
		MyStatic.staticVar=200;
		System.out.println("myStatic.staticVar="+MyStatic.staticVar);
		/*
		 	방법2: 일반적인 멤버변수처럼 참조변수를 통해 접근하는것도가능한다.
		 	단,이경우는 정적변수를 일반적인 멤버변수로 착각할수있으므로 
		 	추천하지않는다.
		 */
		...
		/*
		인스턴스형 멤버변수는 개체별로 각각 존재하므로 아래 초기화된 값은
		객체별로 각각 저장된다.따라서 출력결과는 100,1000이된다.
		 */
		...
		하나만 생성되어 공유되므로 대입문장은 하나의 변수에 값을 할당하게 
		되는것이다. 따라서 결과는 마지막에 입력한 900이 출력된다.
		 */
		...
		myStatic.staticMethod();//권장사항이 아니므로 클래스명을 통해호출하는것이좋다.
		
		//MyStatic.instanceMethod(); 에러발생 참조변수를 통해서만 호출이가능하다. static이아닌
		//void 이니까 참조변수를 통해서 호출을해야하고
		MyStatic.staticMethod();//===>이거는 스태틱이니 바로호출이가능하다.
	}
