...
/*
싱글톤 디자인 패턴:
				클래스를 설꼐하는 디자인패턴의 하나로써
				하나의 인스턴스 즉 하나의 메모리를 생성해 이를 공유해서 사용하고자
				할때 사용하는 패턴이다.
작성방법 
	1.생성자의 접근지정자를 private으로 정의한다.
	2.클래스 외부에서는 생성자를 통해 접근할 수 없으므로
	new를 통해 객체생성은 불가능햊니다.
	3.클래스 내부에 정적메소드로 해당클래스의 객체를
...
class SingleTone {
	...
	/*
	정적메소드로 선언된 getInstance()메소드에서 해당 객체를 외부로 반환한다.
	 */
...
public class E03SingleToneDesignPattern {
	...
	public static void main(String[] args) {
		...
		/*
		생성자를 접근할 수 없는대신 getInstance()메소드를 통해서만 
		객체의 참조값을 반환받을수 있다. 반환받을떄는 항상 같은 객체를
		반환받게 된다.
		 */
