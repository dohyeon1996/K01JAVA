...
상속(Inheritance)
	...
	부모클래스를 자식클래스가 상속할때는 extends키워드를 사용한다.
 */
//부모클래스(상위,기본,super):사람의 일반적인 형태를 추상화 한클래스.
class Man{
	private String name;
	//생성자는 오버로딩 하여 정의됨
	public Man() {
...
//자식클래스(하위,유도 ,sub) : Man 클래스를 상속받는 회사원 클래스
class BusinessMan extends Man {
	private String company;
	private String position;
	/*
	상속받는 자식클래스는 부모클래스의 생성자를 호출하여 부모객체가
	먼저생성되도록 해야한다.
	super()는 부모클래스의 생성자를 호출하는 역활을 하게되면
	만약 super()를 기술하지 않으면 디폴트(기본)생성자가 호출된다.
	 */
	
	public BusinessMan(String name,String company,String position) {
		super(name);//부모클래스의 매개변수 1개인 생성자를호출한다.
...
public class E01BusinessManMain {
	...
	public static void main(String[] args) {
		/*
		자식 클래스를 통해 객체를 생성하지만,실행결과에는 
		부모클래스의 생성자가 먼저호출된다. 항상 부모객체가 먼저 생성된후 자식
		객체가 생성된다.
		 */
