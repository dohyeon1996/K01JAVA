...
class Account {
	...
	/*
	입금처리:protected로 선언되었으므로 상속 관계에서 접근가능
	 */
	protected void depositMoney(int _money) {
		if(_money<0 ) {
			System.out.println("마이너스금액은 입금처리 불가합니다");
			return;
		}
		money+=_money;
	}
	//private으로 선언된 멤버변수를 외부로 반환할때 사용한다.
...
class SavingAccount extends Account {
	/*
	인자생성자에서 부모의 생성자 메소드를 호출하기위해 super()를 사용한다.
	이때 인자가 하나인 생성자를호출한다.
	 */
	...
	public void showAccountMoney() {
		/*
		private멤버변수인 money는 자식쪽에서 직접접근할수 없으므로 접근가능한
		멤버메소드를 통해 간접적으로 접근한다.
		 */
