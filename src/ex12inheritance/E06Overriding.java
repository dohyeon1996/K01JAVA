...
/*
오버라이딩(Overriding)
	:클래스가 상속관게에 있을때 하위클래스에서 상위클래스의 멤버메소드와
	완전히 동일한 모양으로 재정의 하는것을 말한다.
	오버로딩(Overloading)과 다른점은 매개변수의 갯수,반환타입
	등이 완전히 똑같은 형태로 정의한다는것이다.
	하위클래스에서 오버라이딩한 메소드는 상위클래스의 메소드를 
	가리게 되므로 항상 최하위 클래스에서 오버라이딩한 메소드가 호출되게 된다.
 */
//일반적인 스피커를 추상화한 클래스
class Speaker {	
	private int volunmRate;
	//외부에서 설정 가능한 setter 메소드 
	public void setVolumn(int vol) {
		volunmRate=vol;
	}
	//상속관계에서 오버라이딩을 목적으로 정의한 메소드
	public void showStrate() {
		System.out.println("스피커의 볼륨크기"+volunmRate);
	}
}
//일반적인 스피커 기능에 중저음이 보강된 스피커를 추상화
class BaseSpeaker extends Speaker{
	private int baseRate;
	public void setBase(int bas) {
		baseRate=bas;
	}
	/*@Override:'어노테이션'이라 부르고 오버라이딩된 메소드에 추가하여 컴파일러에게 
	 * 알려주는 역활을 한다.
	 * 
	 */
	@Override
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		baseSpeaker.showStrate();
		/*
		자식 객체로 호출하였으므로 자식 클래스에서 오버라이딩한 showstate()메소드가
		호출된다.
		 */
		System.out.println();
		/*
		...
		호출할 수 없다. setBase()는 자식 클래스에서 정의된 멤버메소드이기때문이다.
		단.오버라이딩 처리된 메소드가 있는 경우에는 하위클래스의 메소드가 호출된다.
		 */
