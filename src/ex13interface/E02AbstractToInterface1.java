...
추상클래스는 대부분 추상메소드를 포함하고 있으므로 이를 상속받는 클래스는 반드시
추상메소드를 오버로딩해야한다.
-클래스의 상속관ㄱ에서의 설계도 역활을 한다.
-객체생성은 할수 없으나 그외 참조변수,상속등으 ㅣ모든기능을 가지고있다.

추상메소드
-오버라이딩의 목적으로 제작된다.
함수의 실행부가없기때문에 {}를 쓰지않고 원형에 ;을 붙여준다
 */
...
/*
DTO(Data Transfer Object) :데이터를 저장하고 전송하기위한
용도의 객체를 말한다. VO(value Object)로 표현하기ㄷ한다. 
해당프로그램에서는 한사람의 정보를 저장하기 위한 용도로 정의하엿따.
 */
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs {
	...
	/*
	상속을 통해 추상메소드를 포함하게 되었으므로 반드시 아래와 같이 오버라이딩을
	해야한다. 필수사항이며 하지 않을경우 에러가 발생한다. 
	 */
