...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		/*
		아래 2개의 컬렉션은 내부적인 자료구조만 다를뿐,구현방법은 완전히 동일하다.
		...
		Student st3=new Student("장돈건", 30, "2016");
		...
		/*
		객체삭제 : 인덱스가 아닌 인스턴스의 참조값을 통해 삭제하는경우
			중복된 객체가 있으면 앞에있는 객체 하나만삭제된다. 그래서원빈이삭제됨
		 */
		list2.remove(st2);
		...
		/*	Student student=it2.next();
			student.showInfo();
		위두줄의 코드를 아래 1줄로 고칠수잇다	*/
		it2.next().showInfo();
		}
		
		//List계열의 컬렉션은 인덱스로 접근가능함
		System.out.println("[일반 for문사용]");
		...
		//private멤버라서 getter메소드를 이용해서 접근해야한다. 
		//인스턴스 참조값을 통한삭제
		list2.remove(st1);
