...
public class DBconnect {
	...
	public static void main(String[] args) {
		try {
			...
			:new를 사용하지 않고 클래스명으로 직접찾아서 객체를 생성핞 ㅜ
			메모리에 로드하는 forName()메소드를 사용한다.
			메모리에 로드된 드라이버가 DriverManager라는클래스에 등록된다.
			 */
			Class.forName("oracle.jdbc.OracleDriver");
			/*
			2.오라클 연결을 위한 커넥션 URL,계정아이디,패스워드를 준비한다.
			커넥션 URL=> jdbc:oracle:thin:@IP주소:포드번호:SID명
			※서버환경에 따라 ip주소,포트번호,sid는변경될수있따.
			 */
